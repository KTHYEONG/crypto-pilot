...
- **Conceptual Distinction & Surgical Boundaries (Crucial)**:
  - **Architecture (docs/architecture/)**: **"The What/Current Static State"** (SSOT).
    ...
    - **금지 사항 (CRITICAL)**: 구현 디테일, 작업 히스토리, 버그 수정 로그, 개발 비화 추가 불가. **`[ADR_...]` 태그, 날짜 언급(예: "2026-07에 추가됨"), 변경 이력 서술을 아키텍처 문서 내에 절대 삽입하지 말 것.** 이들은 오직 `decisions.md`에만 격리되어야 함.
    - **수정 기준**: 시스템의 정적 구조(수식, 공용 I/O 계약)가 실제로 변경된 경우에만 최소한의 정적 업데이트 적용.
