...
trigger:
  ...
  - on_file_path_regex: "src/.*(mhs|quant|market_data|pipeline|evaluation|storage|strategy|engine).*"
  - on_file_path_glob: ["src/mhs/**/*.py", "src/quant/**/*.py", "src/market_data/**/*.py", "src/pipeline/**/*.py"]
priority: 10
---
...
- **Escalate When Justified:** If a measured hot path cannot be efficiently handled with standard vectorization (e.g., sequential state-dependent recurrence, matching loop), evaluate JIT compilation, native code, out-of-core engines, or parallelism based on the bottleneck shape. Adopt added complexity only when benchmarks demonstrate meaningful gains.
