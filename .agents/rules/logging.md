...
trigger:
  - on_label: ["logging"]
  - on_file_path_regex: "src/.*(logging|engine|execution|api|pipeline|service|orchestration|market_data).*"
  - on_file_path_glob: ["src/**/logging/**/*.py", "src/**/execution/**/*.py", "src/**/market_data/**/*.py"]
priority: 9
---
...
- **Strict Credential Redaction:** NEVER log exchange API keys, private secret keys, passphrase tokens, webhook credentials, or sensitive wallet/account information.
...
- **Large Collection Summaries:** Never dump massive arrays, DataFrames, or raw order book depth directly into logs. Summarize collections using concise descriptors: count, shape, min/max range, null/nan count, or representative head/tail samples.
...
- **Operational vs. High-Frequency Logs:** Keep high-frequency streaming data (e.g., WebSocket ticks, order fills, raw optimizer trials, detailed memory snapshots) isolated in dedicated log destinations (e.g., specific `.jsonl` or diagnostic artifacts) rather than spamming primary system logs.
- **INFO vs. DEBUG/TRACE:**
