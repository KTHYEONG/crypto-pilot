...
2. **Artifact Cleanup**:
   - Purge temporary `docs/specs/` files, `scratch/` test scripts, `logs/` run logs, and the `tmp/` project-only temp root (pytest/tempfile/TMPDIR artifacts pinned via `tests/conftest.py`).
