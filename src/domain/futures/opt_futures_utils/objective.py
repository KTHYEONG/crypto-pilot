...
def objective_futures(
    ...
    *,
    ...
    for path_idx, path in enumerate(cpcv_paths):
        ...
        for test_start, test_end in path:
            if mode == "multi":
                ...
                win_rate_s = float((len(trades_df[true_pnl > 0]) / len(trades_df)) * 100) if len(trades_df) > 0 else 0.0
                ...
                sum_ff = float(trades_df["funding_fee"].sum()) if "funding_fee" in trades_df.columns else 0.0
            ...
            for sym in symbols:
                ...
                if mdd_pct >= liq_mdd_thr: raise optuna.TrialPruned()
            ...
            if np.sum(sym_trades) < min_seg_trades: raise optuna.TrialPruned()
        ...
        if mode == "multi":
            path_terminal_wealth_ratios.append(float(running_balance / float(FUTURES_INITIAL_BALANCE)))

        if not seg_raw_logs: raise optuna.TrialPruned()
    ...
    p10_log_tw = float(np.percentile(path_arr, 10.0)) if path_arr.size >= 10 else float(np.mean(path_arr))
    ...
    mu_paths, sd_paths = float(np.mean(path_arr)), float(np.std(path_arr, ddof=1)) if path_arr.size > 1 else 1.0
    ...
    if len(path_compound_raw_log_tw) >= 4:
        ...
        d_temporal = float(np.clip(1.0 - (max(0.0, mu_paths - recent_mean) / (abs(mu_paths) + 1e-6)), 0.0, 1.0))
    ...
    if path_arr.size >= 2:
        ...
        if psort > 1.5: stability_bonus += min(0.1, (psort - 1.5) * 0.05)

    objective_final = p10_gmgr * (d_fund * d_balance * d_stability * d_temporal * d_cvar) + stability_bonus
    ...
    from config.settings import SLIPPAGE_RATE, TRADING_FEE_RATE
    ev_cost_is = (avg_ret_trade / ((TRADING_FEE_RATE * 2.0) + (SLIPPAGE_RATE * 2.0))) if avg_trades > 0 else 0.0
    ...
    tw_ratios = np.asarray(path_terminal_wealth_ratios, dtype=np.float64) if mode == "multi" else np.exp(np.clip(path_arr, -50.0, 50.0))
    trial.set_user_attr("min_path_terminal_wealth_ratio", float(np.min(tw_ratios)) if tw_ratios.size else 0.0)
    trial.set_user_attr("mean_path_terminal_wealth_ratio", float(np.mean(tw_ratios)) if tw_ratios.size else 0.0)
    ...
    if path_arr.size >= 2:
        ...
        trial.set_user_attr("gate1_path_sortino", float(m_pt / (ddev + 1e-12)) if ddev > 0 else 999.0)
        pos_only = path_arr[path_arr > 0]
        neg_mean = float(np.mean(neg_only)) if neg_only.size else -1e-9
        tr_val = float(np.mean(pos_only) / (abs(neg_mean) + 1e-12)) if pos_only.size and neg_only.size else (10.0 if pos_only.size else 0.0)
