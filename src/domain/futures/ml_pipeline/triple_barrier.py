...
def _tbm_inner_loop_numba(
    ...
    """Numba-accelerated inner loop for TBM labeling."""
    n4 = len(dt4)
    labels = np.zeros(n4, dtype=np.float64)
    for i in range(n4 - 1):
        entry = open_[i]
        a = atr[i]
        # Fallback if ATR is not yet calculated (early bars)
        ...
        # Use binary search to find the slice of 1m data (O(log M) instead of O(M))
        ...
        # Limit scan by time_stop_bars (e.g. 1440 mins = 1 day)
        ...
        for j in range(idx_start, max_scan):
            hi = m1_high[j]
            lo = m1_low[j]
            # Prioritize SL if hit in same bar (conservative)
            if lo <= sl:
                hit_sl = True
                break
            if hi >= tp:
...
def label_triple_barrier(
    ...
    Returns 1 if TP first, else 0.
    """
    if df_4h.empty or df_1m.empty or len(df_4h) < atr_period + 2:
        return pd.Series(dtype=np.float64)

    # Sort to ensure binary search works on timestamps
    ...
    # ATR calculation on 4h resolution
    atr4 = compute_atr_numpy(high4, low4, close4, atr_period)
    
    # Accelerated inner loop execution
