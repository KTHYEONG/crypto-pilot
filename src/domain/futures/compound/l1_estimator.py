...
def _ridge_slope(
    ...
    se = np.sqrt(residual_var / (n_eff * np.var(x_masked) + 1e-15))
    se * se
...
def estimate_cross_fitted_alpha_tape(
    *,
    ...
    forecast_var = np.full((n_bars, n_syms, n_recipes), 1e-4, dtype=np.float32)
    reliability = np.zeros((n_bars, n_syms, n_recipes), dtype=np.float32)
    valid = raw.valid_3d.copy()

    for k in range(n_recipes):
        recipe_slopes: list[float] = []
        label_matrix = _compute_labels(cube, slice(0, n_bars), raw.horizon_bars_1d[k])
        for fold in folds:
            ...
            for n in range(n_syms):
                ...
                beta, n_eff, res_var = _ridge_slope(x_fit, y_fit)
                if n_eff > 0:
        ...
        for fold in folds:
            ...
            for n in range(n_syms):
                ...
                se_beta = np.sqrt(res_var / (n_eff * np.var(x_fit) + 1e-15)) if n_eff > 3 else 1.0
                param_var_oos = se_beta * se_beta * x_oos * x_oos
                var_pred = res_var + param_var_oos

                gross_mu[oos_slice, n, k] = mu
                forecast_var[oos_slice, n, k] = np.where(var_pred > 0, var_pred, 1e-4)
                reliability[oos_slice, n, k] = np.float32(min(1.0, n_eff / config.active_effective_n))

    lifecycle = _compute_lifecycle(gross_mu, forecast_var, config)
...
def _compute_lifecycle(
    gross_mu: NDArray[np.float32],
    forecast_var: NDArray[np.float32],
    config: L1EstimatorConfig,
) -> tuple[AlphaLifecycle, ...]:
    n_recipes = gross_mu.shape[2]
    lifecycle: list[AlphaLifecycle] = []
    for k in range(n_recipes):
        mu_k = gross_mu[:, :, k].ravel()
        var_k = forecast_var[:, :, k].ravel()
        valid_mask = np.isfinite(mu_k) & np.isfinite(var_k) & (var_k > 0)
        n_eff = float(np.sum(valid_mask))
