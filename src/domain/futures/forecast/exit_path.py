...
@njit(cache=True)
def _label_kernel_core(  # type: ignore[no-untyped-def]
    entry_idx,
    decision_idx,
    side_arr,
    horizon_bars,
    ...
    costs = np.full(n, np.nan, dtype=np.float64)
    funding = np.full(n, np.nan, dtype=np.float64)
    ...
    for i in range(n):
        decision = int(decision_idx[i])
        entry = int(entry_idx[i])
        symbol = int(symbol_idx[i])
        horizon = max(int(horizon_bars[i]), 1)
        if decision < 0 or entry != decision + 1 or entry >= t_len or symbol < 0 or symbol >= open_2d.shape[1]:
            continue
        entry_price = open_2d[entry, symbol]
        atr_val = atr_2d[decision, symbol]
        if not np.isfinite(entry_price) or entry_price <= 0.0 or not np.isfinite(atr_val) or atr_val <= 0.0:
            continue
        side_sign = 1.0 if side_arr[i] > 0 else -1.0
        stop_distance = max(stop_atr_mult[i], 1e-6) * atr_val
        target_distance = max(target_atr_mult[i], 1e-6) * atr_val
        stop_price = entry_price - side_sign * stop_distance
        target_price = entry_price + side_sign * target_distance
        end = min(entry + horizon - 1, t_len - 1)
        scan_from = max(int(min_hold_bars[i]), 1)
        stop_bar = t_len + 1
        target_bar = t_len + 1
        for offset in range(scan_from, end - entry + 1):
            bar = entry + offset
            high = high_2d[bar, symbol]
            low = low_2d[bar, symbol]
            if not np.isfinite(high) or not np.isfinite(low):
                continue
            stop_hit = low <= stop_price if side_sign > 0 else high >= stop_price
            target_hit = high >= target_price if side_sign > 0 else low <= target_price
        ...
        if stop_bar <= target_bar and stop_bar <= t_len:
            exit_bar = stop_bar
            exit_code = 1
            raw_open = open_2d[exit_bar, symbol]
            exit_price = min(raw_open, stop_price) if side_sign > 0 else max(raw_open, stop_price)
            ...
            exit_price = open_2d[next_open, symbol]
        if not np.isfinite(exit_price) or exit_price <= 0.0:
            continue
        gross_return = side_sign * np.log(exit_price / entry_price)
        cost = cost_floor_bps[i]
        if not np.isfinite(cost):
            cost = cost_bps_2d[decision, symbol]
        if not np.isfinite(cost):
            cost = taker_round_trip_bps
        cost = max(cost, taker_round_trip_bps)
        funding_stop = exit_bar + 1
        realized_funding = 0.0
        for f in range(entry, min(funding_stop, funding_2d.shape[0])):
            value = funding_2d[f, symbol]
            if np.isfinite(value):
                realized_funding += side_sign * value * _BPS
        gross[i] = gross_return * _BPS
        costs[i] = cost
        funding[i] = realized_funding
        edge[i] = gross[i] - costs[i] - realized_funding - hurdle_bps[i]
        path_end = min(exit_bar + 1, t_len)
        for bar in range(entry, path_end):
            close = close_2d[bar, symbol]
            if not np.isfinite(close) or close <= 0.0:
                continue
            path_return = side_sign * np.log(close / entry_price) * _BPS
...
def _label_kernel(request: ExitPathRequest) -> tuple[np.ndarray, ...]:
    """Numba-accelerated exit labeling wrapper."""
    return _label_kernel_core(  # type: ignore[no-any-return]
        entry_idx=np.ascontiguousarray(request.entry_idx),
        decision_idx=np.ascontiguousarray(request.decision_idx),
        side_arr=np.ascontiguousarray(request.side),
        horizon_bars=np.ascontiguousarray(request.horizon_bars),
        stop_atr_mult=np.ascontiguousarray(request.stop_atr_mult),
        target_atr_mult=np.ascontiguousarray(request.target_atr_mult),
        min_hold_bars=np.ascontiguousarray(request.min_hold_bars),
        symbol_idx=np.ascontiguousarray(request.symbol_idx),
        open_2d=np.ascontiguousarray(request.open_2d),
        high_2d=np.ascontiguousarray(request.high_2d),
        low_2d=np.ascontiguousarray(request.low_2d),
        close_2d=np.ascontiguousarray(request.close_2d),
        atr_2d=np.ascontiguousarray(request.atr_2d),
        cost_bps_2d=np.ascontiguousarray(request.cost_bps_2d),
        funding_2d=np.ascontiguousarray(request.funding_2d),
        cost_floor_bps=np.ascontiguousarray(request.cost_floor_bps),
        hurdle_bps=np.ascontiguousarray(request.hurdle_bps),
        taker_round_trip_bps=float(request.taker_round_trip_bps),
    )


# Python reference implementation kept for equivalence testing
def _label_kernel_python(request: ExitPathRequest) -> tuple[np.ndarray, ...]:
    ...
    for i in range(n):
        ...
        for offset in range(scan_from, end - entry + 1):
            ...
            if not np.isfinite(high) or not np.isfinite(low):
                continue
            stop_hit = low <= stop_price if s > 0 else high >= stop_price
            target_hit = high >= target_price if s > 0 else low <= target_price
            if stop_hit and stop_bar > t_len:
                stop_bar = bar
            if target_hit and target_bar > t_len:
                target_bar = bar
        ...
        if stop_bar <= target_bar and stop_bar <= t_len:
            ...
            exit_price = min(raw_open, stop_price) if s > 0 else max(raw_open, stop_price)
        elif target_bar <= t_len:
            exit_bar = target_bar
            exit_code = 2
            exit_price = target_price
        ...
        if not np.isfinite(exit_price) or exit_price <= 0.0:
            continue
        gross_return = s * np.log(exit_price / entry_price)
        ...
        for bar in range(entry, path_end):
            ...
            if not np.isfinite(close) or close <= 0.0:
                continue
            path_return = s * np.log(close / entry_price) * _BPS
            if not np.isfinite(mae[i]) or path_return < mae[i]:
                mae[i] = path_return
            if not np.isfinite(mfe[i]) or path_return > mfe[i]:
                mfe[i] = path_return
