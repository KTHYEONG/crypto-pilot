...
class VwapMeanReversionFuturesSignal:
    ...
    param_space: ClassVar[Dict[str, Any]] = {
        ...
        "MR_TREND_EMA_PERIOD": {"type": "int", "low": 20, "high": 100, "step": 20},  # trend filter
    }
    ...
    def compute(self, df: pd.DataFrame, params: Dict[str, Any]) -> FuturesSignalOutput:
        ...
        trend_ema_p = int(params.get("MR_TREND_EMA_PERIOD", 50))
        ...
        # Trend context: EMA filter prevents mean-reversion entries against major trend
        trend_ema = pd.Series(close).ewm(span=trend_ema_p, adjust=False).mean().to_numpy()
        in_uptrend = close > trend_ema
        in_downtrend = close < trend_ema
        ...
        var_vwap = (
            pd.Series(diff_sq).rolling(window=window, min_periods=window // 2).mean().to_numpy()
        )
        ...
        # Signals: only enter in trend-consistent direction
        # Long (oversold) only allowed when price is above EMA (uptrend support)
        # Short (overbought) only allowed when price is below EMA (downtrend resistance)
        long_e = (z_score < -z_threshold) & in_uptrend
        short_e = (z_score > z_threshold) & in_downtrend
