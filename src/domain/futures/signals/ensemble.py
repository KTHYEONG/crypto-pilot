...
from src.domain.futures.signals.contracts import CandidateModelOutput, EdgeSource
...
def _resolve_ensemble_target_column(events: pd.DataFrame) -> str:
    for column in _GROSS_TARGET_COLUMNS:
        if column in events.columns:
            return column
    raise ValueError(f"missing ensemble target column; expected one of {[*_GROSS_TARGET_COLUMNS]}")


def _target_contract_kind(events: pd.DataFrame) -> str:
    return "gross" if any(column in events.columns for column in _GROSS_TARGET_COLUMNS) else "unknown"
