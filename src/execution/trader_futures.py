...
try:
    from src.core.utils.cloud_optimizer import CloudOptimizer
...
class RealTraderFutures:
    ...
    def load_strategies_from_json(self) -> None:
        ...
        if enc_path.exists() or json_path.exists():
            ...
            logger.info("Live multi-portfolio symbols: %s", ", ".join(self.symbols))
    ...
    def _resolve_entry_market_fallback(
        ...
        if mode in ("always_taker", "taker"):
            return True
        vol_pct = ((float(atr) / float(current_price)) * 100.0) if current_price and atr else 0.0
    ...
    def _bootstrap_state_for_open_position(
        ...
        try:
            ...
            if sl_type == "ATR" and entry_atr > 0:
                ...
                stop_price = (
                    entry_price * (1 - sl_pct) if amount > 0 else entry_price * (1 + sl_pct)
                )
            client = self._get_client_for_symbol(symbol)
            ...
            self.state_manager.update_symbol_state(
                symbol,
                {
                    "entry_time": datetime.utcfromtimestamp(now_ref_ms / 1000.0).isoformat(),
    ...
    def _place_order_safe(
        ...
        if order_type.upper() == "LIMIT":
            ...
            try:
                ...
                open_orders = client.fetch_open_orders(exchange_symbol)
    ...
    def _refresh_exit_indicators_if_needed(
        ...
        try:
            ...
            refreshed = {
                ...
                "atr": float(
                    last_candle.get("atr", 0.0) if not pd.isna(last_candle.get("atr")) else 0.0
                ),
                "parabolic_sar": float(
                ...
                "rsi": float(
                    last_candle.get("rsi", 50.0) if not pd.isna(last_candle.get("rsi")) else 50.0
                ),
    ...
    def _execute_entry(
        ...
        if margin_context is not None:
            ...
            used_margin = ((qty * current_price) / leverage) * margin_buffer_multiplier
        ...
        if not self._enforce_min_fill_ratio(
            ...
            logger.info(f"🔄 [{symbol}] Entry attempt refunded due to underfill abort.")
            return True
        ...
        if sl_active:
            sl_order_id = str(sl_result.get("id", "") or "") if isinstance(sl_result, dict) else ""
            if sl_order_id:
                self.state_manager.update_symbol_state(
                    symbol,
                    {
        ...
        try:
            ...
            if not in_position:
                ...
                if state_snapshot and (
                    ...
                    if state_snapshot.get("entry_time") and not state_snapshot.get("exit_pending"):
                    ...
                    logger.info("🧹 [%s] Clearing stale local state (no open position).", symbol)
    ...
    def _log_silent_exchange_exit(
        ...
        try:
            ...
            actual_exit_amount = initial_amount * 0.5 if has_scaled_out else initial_amount
            ...
            if entry_time_str:
                try:
                    ...
                    hold_hours = max(0.0, (datetime.utcnow() - entry_dt).total_seconds() / 3600)
    ...
    def _scan_entries(self, symbol: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if need_calculation:
                ...
                # [FIX 1] Key 매핑 동기화
                vol_ratio = last_candle.get("vol_zscore", last_candle.get("volume_ratio", -10.0))
                ...
                self._cache_indicators(
                    symbol,
                    {
                        ...
                        "entry_upper": (float(entry_upper) if pd.notna(entry_upper) else None),
                        "entry_lower": (float(entry_lower) if pd.notna(entry_lower) else None),
            ...
            successful_entry_ts = int(state.get("successful_entry_candle_ts", 0) or 0)
            ...
            if use_intrabar_entry:
                ...
                long_signal = is_uptrend and strength_ok and (signal_price > float(entry_upper))
                short_signal = is_downtrend and strength_ok and (signal_price < float(entry_lower))
            ...
            candidate: dict[str, Any] = {
                ...
                "entry_post_only_requote_max": int(params.get("ENTRY_POST_ONLY_REQUOTE_MAX", 2)),
    ...
    def _calculate_position_size(
        ...
        if margin_context is not None:
            ...
            total_balance = float(margin_context.get("total_equity_snapshot", usdt_free))
        else:
            try:
        ...
        trading_equity = (
            min(total_balance, max_capital_usage) if use_compounding else max_capital_usage
        )
        ...
        if margin_context is not None:
            ...
            margin_buffer_multiplier = (
                1.0 + (fee_rate * 2.0) + float(SLIPPAGE_RATE) + max_expected_funding_rate
            )
        ...
        try:
            ...
            if (not has_scaled_out) and initial_amount != 0.0:
                if abs(amount) <= abs(initial_amount) * 0.5:
                    scale_out_done = True
                    has_scaled_out = True
                    scale_exit_price = float(state.get("scale_target_price", current_price))
                    scale_qty_raw = max(0.0, abs(initial_amount) - abs(amount))
                    ...
                    if scale_qty > 0:
                        ...
                        if amount > 0:
                            gross_pnl = (scale_exit_price - entry_price) * scale_qty_float
                        else:
                            gross_pnl = (entry_price - scale_exit_price) * scale_qty_float
            ...
            if scale_out_done:
                ...
                if scale_order_id_state:
                    scale_order_id = str(scale_order_id_state.get("scale_order_id", "") or "")
                    if scale_order_id:
                        try:
                            client = self._get_client_for_symbol(symbol)
                ...
                fee_rate = float(params.get("TAKER_FEE_RATE", params.get("FEE_RATE", 0.0005)))
                breakeven_raw = (
                    entry_price * (1.0 + fee_rate * 2.0)
                    if amount > 0
                    else entry_price * (1.0 - fee_rate * 2.0)
                )
                client = self._get_client_for_symbol(symbol)
            ...
            if not exit_triggered and not fallback_without_candle:
                min_update_step = 0.0
                if pos_atr > 0:
                    atr_step_ratio = float(params.get("TRAIL_UPDATE_ATR_RATIO", 0.2))
                        ...
                        if sl_result:
                            ...
                            if prev_stop > 0:
                                ...
                                if sl_result_prev and isinstance(sl_result_prev, dict):
                                    sl_order_id_prev = str(sl_result_prev.get("id", "") or "")
                                    update_payload_prev = {
                                    ...
                                    if sl_order_id_prev:
                                        update_payload_prev["sl_order_id"] = sl_order_id_prev
                                    self.state_manager.update_symbol_state(
            ...
            if closed_qty > 0:
                if not self._place_order_safe(symbol, order_side, closed_qty, reduce_only=True):
                    self.state_manager.update_symbol_state(
                        symbol,
    ...
    def _is_stop_loss_order(o: dict[str, Any]) -> bool:
        ...
        is_stop = (
            ("STOP" in ccxt_type)
            or ("STOP" in raw_type)
            or ("STOP" in orig_type)
            or ("STOP" in algo_type)
        )
        is_take_profit = (
            ("TAKE_PROFIT" in ccxt_type)
            or ("TAKE_PROFIT" in raw_type)
            or ("TAKE_PROFIT" in orig_type)
            or ("TAKE_PROFIT" in algo_type)
        )
    ...
    def _restore_stop_loss(
        ...
        if using_state_stop:
            ...
            if amount > 0:
                sl_side = "sell"
                if sl_type == "ATR" and atr > 0:
                    stop_price = entry_price - (atr * float(params.get("LONG_ATR_MULT", 2.0)))
                else:
                    stop_price = entry_price * (1 - params.get("STOP_LOSS_PCT", 0.02))
            else:
                sl_side = "buy"
                if sl_type == "ATR" and atr > 0:
                    stop_price = entry_price + (atr * float(params.get("SHORT_ATR_MULT", 2.0)))
                else:
                    stop_price = entry_price * (1 + params.get("STOP_LOSS_PCT", 0.02))

        tick_size = float(
            client.get_price_tick_size(symbol, fallback=(0.1 if "BTC" in symbol else 0.01))
        )
        stop_price = max(tick_size, stop_price)
        stop_price = client.round_price(symbol, stop_price)
    ...
    def _recover_pending_exit(
        ...
        client = self._get_client_for_symbol(symbol)
        exit_qty = client.round_amount(symbol, abs(actual_amount)) or abs(actual_amount)
            ...
            try:
                ...
                if entry_candidates:
                    ...
                    for cand in entry_candidates:
                        ...
                        pre_entry_free_usdt = float(margin_context.get("free_usdt", 0.0))
                        try:
                ...
                self.health_manager.update_heartbeat(status="running", positions=positions)
