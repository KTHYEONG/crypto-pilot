...
def main(argv: Sequence[str] | None = None) -> int:
    """Preserve the deployed artifact keygen, seal and unseal interface under source ownership. Secret keys are supplied only through LIVE_ARTIFACT_KEY. Args: existing artifact CLI arguments. Returns: 0 on success and 1 on envelope failure. Raises: SystemExit for missing keys, invalid arguments or file I/O failure."""
    ...
    if args.command == "keygen":
        print(base64.b64encode(secrets.token_bytes(32)).decode("ascii"))  # noqa: T201 - keygen stdout is the tool contract
        return 0
