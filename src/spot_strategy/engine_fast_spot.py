...
class BacktestEngineFastSpot:
    ...
    def _prepare_data_with_index(self):
        """
        [FAST PATH] Optimized data preparation.
        Assume indicators are ALREADY computed in daily_df and hourly_df by the optimizer.
        """
        # 1. Extract Daily Indicators for Numba mapping
        ...
        self.daily_indicators = np.column_stack([
            self._shift_1bar(self._extract_column(self.daily_df, col, default_value=np.nan))
    ...
    def _extract_arrays(self):
        """Extract numpy arrays directly from hourly dataframe."""
        ...
        # [LOOKAHEAD PROTECTION] Shift all hourly signal arrays by 1 bar.
    ...
    def run(self):
        ...
        trades, equity, final_bal = self.backtest_func(
            ...
            self.entry_upper,
            self.hourly_trend_dir, self.strength_filter, self.volume_ratio,
            self.atr, self.parabolic_sar,
            self.hurst_h, self.natr_h, self.rsi,
            self.daily_indicators, self.merge_idx, self._trend_gate_mode_code,
