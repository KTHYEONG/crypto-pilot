...
class CloudOptimizer:
    ...
    def cleanup_db_old_records(self, db_path: Path, table: str = "trades", days_to_keep: int = 90):
        """
        ...
        allowed_tables = {"spot_signals", "futures_signals"}
        if table not in allowed_tables:
            self.logger.error(f"Invalid table name for cleanup: {table}")
            return

        try:
            with sqlite3.connect(db_path) as conn:
                cursor = conn.execute(
                    f"SELECT COUNT(*) FROM {table} WHERE timestamp < ?",  # nosec: S608
                    (cutoff_date,),
                )
                ...
                if old_count > 0:
                    conn.execute(
                        f"DELETE FROM {table} WHERE timestamp < ?",  # nosec: S608
                        (cutoff_date,),
                    )
                    conn.commit()
                    self.logger.info(f"Cleaned up {old_count} old signals from {table}")
