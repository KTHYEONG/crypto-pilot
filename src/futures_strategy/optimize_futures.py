...
from config.optimization_config_ultimate import COMMON_SEARCH_SPACE # Keep for potential shared usage
        ...
        if trial.number > 0 and trial.number % self.interval == 0:
            with self._lock:  # Prevent parallel checkpointing
                ...
                try:
                    ...
                    temp_storage = optuna.storages.RDBStorage(url=f"sqlite:///{temp_path}")
                    optuna.copy_study(from_study_name=study.study_name, from_storage=study._storage, to_storage=temp_storage, to_study_name=study.study_name)
                    ...
                    print(f"💾 Checkpoint: Progress saved to {db_path} (Trial {trial.number})")
...
def objective(trial, strategy_cls, strategy_name, data_maps, search_space, common_search_space):
    """
...
def main():
    ...
    parser.add_argument("--trials", type=int, default=None, help="Number of optimization trials (default: auto-set by mode)")
    parser.add_argument("--symbols", type=str, default="BTC/USDT,ETH/USDT")
    parser.add_argument("--jobs", type=int, default=12)
    parser.add_argument("--mode", type=str, default="DAY", choices=["SCALP", "DAY", "SWING", "ALL"], help="Trading Mode: SCALP, DAY, SWING")
    ...
    MODE_TRIALS_MAP = {
        'SCALP': 3000,  # 3 timeframes (5m,15m,30m), slowest backtest (high data volume)
        'DAY': 2500,    # 3 timeframes (1h,2h,4h), balanced speed
        'SWING': 2700,  # 3 timeframes (4h,1d,3d), fast backtest but needs more trials (fewer trades)
        'ALL': 3000     # Catch-all (highest complexity)
    }
    ...
    try:
        backtest_loop_numba(
            ...
            False, 3.0, # TP
            _dummy_ts, 0.0001, 8, # Funding params
    ...
    try:
        study.optimize(
            lambda trial: objective(trial, UltimateStrategy, f"Ultimate_{mode}", data_maps, search_space, {}),
            n_trials=trials,
            n_jobs=args.jobs,
