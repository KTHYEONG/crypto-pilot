...
def evaluate_library_admission(
    ...
    *,
    ...
    is sufficient. The rolling-v2 path passes
    ``proposal_search == "bounded_family_unique_v2"``: the candidate screen,
    coverage, and pair matrices are identical but exact enumeration is skipped
    (proposals are generated by
    ...
    """
    if proposal_search not in ("exact_legacy_v1", "bounded_family_unique_v2"):
        raise ValueError(
            f"proposal_search must be 'exact_legacy_v1' or "
            f"'bounded_family_unique_v2', got {proposal_search!r}"
        )
    ...
    if proposal_search == "bounded_family_unique_v2":
