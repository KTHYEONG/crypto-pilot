    ...
    if mark_mode in ("cache_required", "cache_required_stale_carry"):
        if roster:
            ...
            if mark_mode == "cache_required":
                integrity._assert_cache_required_marks("window", piece_weights[roster], piece_signals, minute_marks)
        else:
...
def _iter_mhs_execution_windows(
    ...
    *,
    ...
    Args:
        target_weights: Ordered canonical decision targets.
        signal_available_at: Aligned signal publication timestamps.
        root: Existing execution OHLCV root.
        timeframe: Three-minute execution interval only.
        start: Timezone-aware inclusive evaluation start.
        end: Timezone-aware exclusive evaluation fence.
        funding_by_symbol: Existing funding publication inputs.
        mark_mode: Explicit existing mark provenance policy.
        spec: Existing execution costs and order timeout semantics.
        funding_failures: Per-symbol source failures.
        required_symbols: Live inventory and order requirements across bounds.
    ...
    Raises:
        DataIntegrityError: Bounds, decisions or required provenance conflict.
    """
