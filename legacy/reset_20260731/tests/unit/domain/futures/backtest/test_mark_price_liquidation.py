...
class TestMarkPriceLiquidation:
    ...
    def test_exec_low_below_liq_but_mark_above_no_liquidation(self) -> None:
        ...
        assert equity_mark[-1] >= equity_no_mark[-1], "mark_price 기준이 exec_low 기준보다 equity가 낮을 수 없음"
    ...
    def test_mark_below_liq_triggers_liquidation(self) -> None:
        ...
        assert final_bal_mark <= inputs["initial_balance"] * 1.1, "mark_price 기준 청산이 발생해야 함"
