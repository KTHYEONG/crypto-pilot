...
from src.domain.futures.optimization.opt_config import (  # noqa: E402
    OPT_SPOT_CONFIG,
    SPOT_ANCHOR_SYMBOLS,
    get_quarterly_window,
    get_search_space_spot,
)
from src.core.settings import SPOT_INITIAL_BALANCE  # noqa: E402
...
def main() -> None:
    ...
    if not pre_args.skip_universe:
        ...
        importlib.reload(src.domain.futures.optimization.opt_config)
        globals()["SPOT_SYMBOLS"] = src.domain.futures.optimization.opt_config.SPOT_SYMBOLS
        globals()["OPT_SPOT_CONFIG"] = src.domain.futures.optimization.opt_config.OPT_SPOT_CONFIG
        ...
        if not pre_args.skip_stage1 and pre_args.mode == "multi":
            ...
            if not tops:
                ...
                return
            max_per_sig = int(
                src.domain.futures.optimization.opt_config.OPT_SPOT_CONFIG.get("SPOT_STAGE2_MAX_PER_SIGNAL_TYPE", 2)
            )
    ...
    parser.add_argument("--symbols", type=str, default=",".join(src.domain.futures.optimization.opt_config.SPOT_SYMBOLS))
    parser.add_argument(
