...
def evaluate_library_admission(
    ...
    *,
    ...
    generated by :func:`priority_shortlist_family_unique_proposals`). A
    malformed panel or misaligned context fails closed with an exception;
...
class _WorstKeptFirst:
    """Min-heap wrapper that surfaces the worst-ranked kept leaf at the root.

    ``heapq`` is always a min-heap; inverting ``__lt__`` turns it into a bounded
    max-heap of the current top-``n`` kept leaves, so both admitting a better
    leaf and evicting the current worst are ``O(log top_n)`` instead of the
    ``O(top_n)`` cost of appending to a list and re-sorting it on every insert.
    """

    __slots__ = ("rank", "subset")

    def __init__(
        self, rank: tuple[float, float, float, float, str], subset: tuple[int, ...],
    ) -> None:
        self.rank = rank
        self.subset = subset

    def __lt__(self, other: _WorstKeptFirst) -> bool:
...
def _priority_top_n_for_size(
    ...
    Explores the same family-unique, pair-compatible lattice as the canonical
    increasing-index enumeration (one family per subset) but pops partials off
    a priority queue in ascending order of their current
    ``(max_abs_pair_log_return_correlation, max_pair_joint_negative_rate)``
    bound instead of lexical index order. A partial's current max terms are a
    mathematically valid lower bound for any completion's final rank-key max
    terms, because adding a member only adds new pairs and ``max`` is
    non-decreasing under set union; this licenses safe pruning: a partial is
    dropped without expansion once its bound already loses lexicographically
    (on the max-correlation/max-joint pair) to the current ``top_n``-th kept
    leaf of this size.
    ...
    while heap:
        ...
        worst = found[0].rank if len(found) >= top_n else None
        ...
        if psize == size:
            ...
            if len(found) < top_n:
                heapq.heappush(found, _WorstKeptFirst(rank, partial))
            elif rank < found[0].rank:
                heapq.heapreplace(found, _WorstKeptFirst(rank, partial))
            continue
    ...
    return (
        tuple(item.subset for item in found),
...
def _check_contract() -> None:
    ...
    assert priority_shortlist_family_unique_proposals.__name__ == (
