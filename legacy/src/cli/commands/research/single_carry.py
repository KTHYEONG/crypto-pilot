...
from src.application.research.carry import evaluation as evaluation_module
...
def add_single_carry_commands(run_sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    """Attach the ``research run single carry`` subcommand."""
    cash_carry = run_sub.add_parser(
        "carry", help="Run a sealed cash-and-carry evaluation",
    )
