...
7. **디스크 Tail 증분 갱신 & 원자적 프루닝**
   * *구현*: 디스크 tail 기준 미수집 구간만 증분 패치하고, 430일 초과 시세는 원자적 임시 파일 대체(`tmp.replace`)로 안전 절단.
   * *효과*: 갱신 시간 29분 $\rightarrow$ 약 2분으로 단축, OCI A1.Flex(2 OCPU/12GB) 컨테이너 `mem_limit: 3g` 예산 내 무인 운용.
8. **SHA-256 불변 파라미터 봉인 & 제로 트러스트 배포**
   * *구현*: 전략 파라미터 SHA-256 봉인 검증(`LIVE_ARTIFACT_KEY`), 서버 배치 `.env`를 정본으로 사용, 인바운드 포트 없는 Tailscale 사설망 배포.
...
| **ADR-05** | **저사양 클라우드 운용** | **Tail 증분 동기화(~2분) & 원자적 프루닝(430일 보존)** | 650개 전종목 전수 수집, 수동 디스크 청소 | 갱신 시간 29분 $\to$ 약 2분으로 단축, OCI A1.Flex(2 OCPU/12GB) 컨테이너 `mem_limit: 3g` 예산 내 무인 운용 |
...
### Decision 5: 디스크 Tail 증분 갱신 및 원자적 보존 프루닝 (OCI A1.Flex Cloud Ops)
* **Decision**: 무거운 DB(TimescaleDB) 대신 로컬 Parquet 스토리지와 disk-tail 증분 갱신, 430일 초과 시 원자적 temp-replace 프루닝 구축.
* **Why**: 650개 선물 심볼 데이터를 OCI Ampere A1.Flex(2 OCPU/12GB 호스트 공유) 위 `mhs-live` 컨테이너(`mem_limit: 3g`)에서 OOM 없이 24/7 무인 가동하기 위함. 컨테이너 한도는 고정값이 아니라 신호 패널 확장 등 실측 피크에 맞춰 재산정함.
* **Trade-off**: 복잡한 애드혹 SQL 쿼리는 제한되나, 갱신 시간을 29분에서 약 2분으로 단축.
