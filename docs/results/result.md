...
1. **발견**: 정책을 "만들 때"와 "쓸 때"가 서로 다른 곳에서 타임프레임(TF) 정보를 가져오고 있었다. 만들 때는 정확한 TF 값("8h" 등)을 썼는데, 쓸 때는 전략 이름 문자열에서 TF를 추측하는 방식을 썼다 — crisis 리플레이의 전략 이름에는 그 추측 근거가 아예 없어서 항상 "모름" 처리가 됐다. 그 결과 직전 섹션에서 이식한 정책이 **한 번도 제대로 매칭되지 않고 전부 스쳐 지나갔던 것** — "라우팅이 이식됐다"는 로그는 맞았지만, 실제로는 조회할 때마다 못 찾고 넘어가고 있었다.
2. **추가 확인**: "2022년 위기 구간이 처음부터 끝까지 한 가지 장세뿐이라 애초에 구분할 게 없었을 것"이라는 가설도 직접 계측해서 반증했다 — 실제로는 하락장 53%, 위기장 31%, 상승장 16%로 섞여 있었다. 즉 라우팅이 제대로 매칭되기만 하면 효과를 낼 조건은 충분했다.
3. **수정**: 정책을 쓰는 쪽도 정책을 만들 때와 똑같은 정확한 TF 정보를 쓰도록 통일했다. 정확히 일치하는 게 없을 때는(예: crisis 리플레이가 8h 단일인데 study 정책에 그 TF가 없는 경우) 무작정 포기하지 않고, 같은 장세·같은 전략군에서 표본이 가장 많은 TF의 정책을 대신 쓰도록 보완했다(새로운 규칙을 지어낸 게 아니라 "가장 많이 관찰된 것을 대표로 쓴다"는 원칙만 적용).

`/check` 전체 통과(spec 필수 테스트 검증 포함, lint+mypy+테스트, Cov 39%). 부수 발견 2건도 함께 수정: 신규 코드의 변수명 충돌로 인한 타입 오류, 그리고 무관하게 이미 깨져 있던 기존 테스트 픽스처(BTC 심볼 누락) 1건.

## 실측 결과 (`--phase l3 --seed 42`, 내부 42/43/44, 기본 설정)

로그로 정책이 실제로 매칭되는 것 확인: `policy_cells=36`(정확 매칭 18 + 대체 매칭 18) — 직전엔 이식은 됐지만 매칭이 전부 실패해 사실상 무정책이었다.

| Seed | joint_feasible | crisis_mdd | crisis_cagr | recent_fold | 최종 판정 |
|---|---|---|---|---|---|
| 42 | **1/120 (세션 최초)** | 112 | 95(이전 107) | 6(이전 11) | `blocker=cagr` — no_feasible_trials 아님! |
| 43 | 0/120 | 112 | 96 | 21 | no_feasible_trials |
| 44 | 0/120 | 118 | 102 | 13 | no_feasible_trials |

## 판정

1. **드디어 숫자가 움직였다.** tf 키 불일치가 crisis 측정을 무력화시키던 진짜 원인이었음이 실측으로 확정됐다.
2. **seed=42는 "통과 가능한 조합이 아예 없음"에서 "통과는 했는데 다른 기준(CAGR 상대 게이트)에 걸림"으로 실패 사유 자체가 바뀌었다** — 질적으로 다른, 더 나은 실패다.
3. **여전히 2/3 seed 합의는 못 채웠다**(`admitted=False`) — seed 43·44는 아직 완전 실패. 하지만 이제 라우팅이 실제로 작동하므로, 직전 두 세션의 side-split·regime-scoped override를 이번 수정과 함께 켜면 추가 개선 여지가 있다(측정 안 함, 다음 우선순위).

## 다음 우선순위

- side-split + scoped-override + 이번 tf-key SSOT 수정을 **3종 조합**으로 실측해 seed 43·44도 개선되는지 확인.
- seed=42의 `cagr` blocker(상대 게이트)가 어느 정도 차이로 막혔는지 세부 확인 — 근소한 차이면 추가 조정 여지, 큰 차이면 다른 병목일 가능성.

---

# L2 Crisis Replay Routing Parity 수정 — 2026-07-21

`docs/specs/l2-crisis-replay-routing-parity.md` 구현(`/check` PASS, Cov 46%) — 현재 1위 blocker인 crisis 제약(crisis_mdd/crisis_cagr)을 계산하는 crisis 리플레이가 **라우팅이 통째로 빠진 다른 전략을 평가하고 있던 측정 정합성 결함**을 수정했다.

## 무엇을 했나 (쉽게 설명)

1. **발견**: study(정상장 2025년 구간)를 평가할 땐 "장세별로 다르게 배분하는" 라우팅 로직이 적용되는데, crisis(2022 LUNA/FTX 붕괴) 구간을 평가할 땐 이 라우팅이 아예 빠져 있었다. 즉 정상장에서는 똑똑하게 배분하는 전략을 테스트하면서, 위기장에서는 롱/숏 안 가리는 다른(더 멍청한) 전략을 테스트하고 있었던 셈 — 같은 전략을 테스트해야 공정한 비교인데 그게 아니었다.
2. **수정**: study에서 학습한 라우팅 규칙(가장 오래 학습한 마지막 구간 것 — 실제 배포 시점과 가장 가까움)을 crisis 리플레이에도 그대로 이식했다. 위험 예산(위기장 최대 손실 한도 등)이나 통과 기준은 전혀 안 건드렸다 — 오직 "무엇을 테스트하는지"만 정확하게 맞췄다.

`/check` 전체 통과(spec 필수 테스트 검증 포함, lint+mypy+테스트, Cov 46%).

## 실측 결과 (`--phase l3 --seed 42`, 내부 42/43/44)

- 로그로 라우팅이 실제로 이식되는 것 확인(`policy_cells=18`→side-split 켜면 36으로 정확히 2배 — 배선 정상).
- **그런데 crisis_mdd/crisis_cagr 위반 개수 자체는 수정 전후로 완전히 동일했다.** 두 가지 조합(기본 설정 단독 / side-split+scoped-override 조합) 모두에서 수정 전 수치와 정확히 일치.

## 판정

- **측정은 이제 정확해졌지만, 아직 결과 개선으로는 이어지지 않았다.** 정직하게 말하면 "숫자가 하나도 안 바뀐" 실측 결과다.
- 추정 원인(미확정, 다음 세션 조사 후보): 2022 LUNA/FTX 윈도우 자체가 거의 시종일관 "crash" 한 가지 장세였을 가능성 — 그렇다면 "장세별로 다르게 배분"하는 로직이 적용돼도 애초에 구분할 장세가 하나뿐이라 효과가 안 나타나는 게 당연하다.
- crisis 리스크 예산이 여전히 최다 차단 사유라는 기존 결론 자체는 이제 **믿을 수 있는 측정**에 근거하게 됐다 — 이건 실질적 진전이다(전에는 "잘못된 전략을 테스트해서 나온 결론"이었을 위험이 있었음).

## 다음 우선순위

- 2022 crisis 윈도우의 실제 regime 분포(crash/bear/bull 비중)를 계측해 "라우팅이 발화할 기회가 있었는지" 확인할지, 아니면 여기서 멈추고 L1 알파 확충으로 방향을 돌릴지 — 사용자 판단 필요.

---

# L2 RC-3 Regime-Scoped Fold Override + Phase B 배선버그 완전 수정 — 2026-07-21
...
- **알파 존재 여부**: 반증됨(부재 아님) — bear-short, crash-long 셀에서 실측 LCB 양수 엣지 확인(비용 대비 10배 이상), crisis 윈도우 자체도 bear 53%/crisis 31%/bull 16%로 라우팅이 발화할 조건 충분함을 계측으로 재확인.
- **정상장 CAGR 게이트**: Phase A로 상대 기준 전환, 병목 아님(Best CAGR 31%+ 도달 확인).
- **Crisis 리스크 예산**: 고정 불변 유지 — 여전히 최다 blocker 중 하나지만, 측정 정합성(라우팅 이식 + tf 키 SSOT)이 이번 세션에 완전히 수정되어 신뢰도 높은 측정에 기반.
- **Phase B(side-split) + RC-3 override + Crisis Parity + TF-Key SSOT**: 4단계 배선/측정 결함(precompute 항상 False 고정 → 4-key/3-key shape mismatch → crisis 라우팅 미이식 → 이식됐지만 tf 키 불일치로 100% miss)을 순차적으로 발견·수정 완료. **세션 최초로 seed=42에서 joint_feasible=1 확보** — 실패 사유가 `no_feasible_trials`(통과 가능한 조합 자체 없음)에서 `cagr` blocker(통과는 했으나 다른 기준에 걸림)로 질적 전환.
- **아직 배포 가능한 champion 없음**(2/3 seed 합의 미달) — 그러나 병목이 "알파 부재" → "미배선" → "표본 분할 통계 불안정" → "크라이시스 측정 자체가 무효" → 현재는 **"측정이 정확해진 상태에서의 실제 위험예산-노출 트레이드오프"**로 계속 정교화됨. 다음 우선순위: side-split+scoped-override+tf-key SSOT 3종 조합 실측으로 seed 43·44 개선 여부 확인.
