...
[BRIDGE SUMMARY] Active Signals 0 (sel=0) | Status BLOCKED | Execution Time 30.05s
...
## 다음 단계
1. `selection_min_expected_utility_bps`와 `selection_gate_mode` 조합을 완화해서 `eligible=0` 원인을 분해해야 합니다.
2. `utility_topk`의 `expected_utility_bps` 구성요소를 fold별로 더 촘촘히 기록해서, gate 경직인지 q10/downside 과보수인지 분리해야 합니다.
