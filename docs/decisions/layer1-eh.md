...
- **아키텍처 결정**: `evaluate_outer_signal_opportunities` 내 기존 probe diag hook 직후 동일 가드(`L1_XS_SPREAD_DIAG` env + DEBUG level)로 삽입. `Layer1FoldReadiness` 반환값/blockers/passed 불변 (probe diag와 동일 ADR-050 규약). `realized_event_results`는 이미 함수 파라미터 — 신규 배선 불필요.
- **Env gate 패턴**: `_l1_probe_diag_enabled`와 동일 규약 (`""`/`"0"`/`"false"`/`"False"` → disabled). `archetype` 컬럼 부재 시 `xs_family_fallback` tuple로 방어.
- **판정 분기**: 스프레드 Sharpe/LCB 양(+) → 후속 spec이 portfolio-level XS sleeve 배포. ~0 → 데이터확장/horizon이 유일경로 + XS family revert 근거.
- **Test Coverage**: 15 tests across 9 scenarios (positive/null factor 분리, env gate BVA, min_bars guard, XS 부재 None, family fallback, rank-IC 방향성, constant zero-div, LCB 결정성, format token). All PASS. L1: ruff/mypy clean.
