...
- **Shared Computation** (`resolve_cross_tf_shared_context()`): When both `enable_cross_tf_diversity_audit` and `enable_cross_tf_pruning` are active, `assemble_l0_strategy_delivery_manifest()` builds one `CrossTFSharedContext` (canonical context + per-recipe projection cache + per-recipe side/entry cache + upper-triangle-mirrored correlation matrix) and injects it into both `audit_l0_selected_recipe_independence()` and `compute_cross_tf_redundancy()` via `precomputed_shared_context`, eliminating duplicate canonical-context/projection/correlation work between the two. `compute_cross_tf_pair_evidence()` similarly accepts precomputed per-recipe projections/side-entries/correlation to avoid re-deriving them per pair. Guarded by `resolve_effective_memory_budget()`/`admit_memory_stage()` (same budget mechanism as LTF exec_1m streaming) before the cache is built; exceeding budget raises (fail-open, same as other cross-TF `ValueError`s below).
...
- **Pruning Admission** (`enable_cross_tf_pruning`): `assemble_l0_strategy_delivery_manifest()` runs after `run_alpha_foundry_l0_gate_multi_tf()` completes, computing `compute_cross_tf_redundancy()` + `apply_cross_tf_survival_floor()` (per-archetype and per-TF minimum-survivor guarantee, `cross_tf_pruning_min_survivors_per_archetype`/`cross_tf_pruning_min_survivors_per_tf`) to narrow the per-TF `panels_for_l1`/`candidates_for_l1` sets before they reach L1. Fails open: any `ValueError` from the redundancy computation (e.g. `min_common_active_bars` shortfall, or shared-context memory budget shortfall) falls back to the unpruned set. Stage timing is always logged (success or failure) via `setup_logger("opt_main_futures", write_file=False)` as `[SYS] stage=l0_cross_tf_audit`/`stage=l0_cross_tf_pruning`.
