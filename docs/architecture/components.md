...
**Responsibility**  
거래소 엔드포인트 및 Vision 아카이브로부터 다중 해상도 시세(1h/3m OHLCV, 8h 펀딩비, 1h 마크 가격, 청산 스트림)를 수집·캐싱하고, 디스크 tail 기반 증분 갱신과 무손실 원자적 프루닝을 수행하여 저사양 클라우드 환경에서 디스크 용량을 150MB 이내로 유지합니다.
...
**Key implementation**  
- `src/market_data/services/collection.py`
- `src/market_data/services/futures_collection.py:DataCollector`
- `src/market_data/retention.py:prune_market_data`
- `src/live/data_refresh.py:refresh_live_market_data`
...
**Input**  
과거 1시간봉 거래대금(Quote Volume) 시계열, 심볼 상장 이력, 결손 심볼 목록(`SOURCE_GAP_EXCLUDED_SYMBOLS`).
...
**Key implementation**  
- `src/mhs/pipeline/stages/selection.py:select_horizons`
- `src/quant/universe/pit_universe.py:schmitt_trigger_universe`
...
**Key implementation**  
- `src/mhs/committee.py:build_flow_momentum_committee`
- `src/mhs/committee.py:train_evidence_weights`
- `src/mhs/pipeline/stages/committee.py:build_committee`
...
**Key implementation**  
- `src/mhs/regime.py:beta_neutralize_weights`
- `src/mhs/regime.py:crash_regime_tilt_weights`
- `src/mhs/scaling.py:pnl_volatility_target_scale`
- `src/mhs/books.py:portfolio_rebalance_trigger`
...
**Key implementation**  
- `src/mhs/execution/ledger.py:simulated_inventory_ledger`
- `src/mhs/execution/strategy_replay.py:strategy_aware_execution_replay`
- `src/mhs/execution/accumulator.py:ExecutionAccumulator`
...
**Responsibility**  
시계열 데이터 누출을 방지하고 다중 가설 검정 과적합을 통계적으로 검증합니다. 168시간(1주) 엠바고가 강제된 16-Fold Anchored Purged Walk-Forward CV를 실행하고, 96개 탐색 시도 경로의 분산/첨도를 보정한 Deflated Sharpe Ratio (DSR) 및 9대 합성 스트레스 시나리오 내구성을 평가합니다.
...
**Key implementation**  
- `src/mhs/evidence.py:phase_1_anchored_purged_folds`
- `src/mhs/evidence.py:deflated_sharpe_ratio`
- `src/mhs/pipeline/stages/fold.py:run_folds`
...
**Key implementation**  
- `src/live/scheduler.py:run_daemon`
- `src/mhs/live_signal_step.py:compute_live_signal_step`
- `src/mhs/live_strategy.py:load_strategy_params`
- `src/live/runner.py:run_shadow_cycle`
...
**Key implementation**  
- `src/live/alerting.py:post_alert`
- `src/live/alerting.py:send_email_alert`
