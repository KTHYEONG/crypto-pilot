...
- **XS Alpha Portfolio-Level Admission Gate (`l1_xs_alpha_admission_enabled`, default `False`)**: `resolve_xs_alpha_admission(diag, cfg)` converts a factor's `XsFactorSpreadDiagnostics` cell (`lcb > l1_breakeven_floor_bps` and `sharpe >= l1_xs_admission_min_sharpe`, default `0.15`) into an `XsAdmissionBasis`. When `compute_symbol_strategy_evidence` receives a non-empty `xs_admission` map, matching `strategy_id` rows have `mean_gross`, `mean_incremental`, `probability_positive`, `lcb_net`, `t_stat`, `p_value`, and `block_tstat` substituted with the factor-level values *before* the standard per-pair `structural_reasons`/FDR/registry-`lcb_pass` gates run — the existing gate logic is otherwise unmodified, so the substitution transparently satisfies all three downstream checks that a per-pair peer-exclusive incremental-edge test structurally cannot pass for a cross-sectional-only signal. Sample-size checks (`insufficient_effective_obs`/`insufficient_folds`, `positive_fold_ratio`) remain pair-level. Wired only at the `deployment_evidence` call site in `allocation/pipeline.py` / `strategy/tiered_workflow/pipeline.py` (the prequential-snapshot call site is intentionally left unwired — its event frame lacks `realized_side_adjusted_gross_bps`). `[ADR_20260703_L1_XS]` Economic replay (L1→L2→L3, flag on vs off) confirmed the gate engages (36→232 promoted pairs) but degrades L3 holdout (CAGR -11.3%→-17.7%, Sharpe -0.860→-1.232) despite improving L2 fit/cal — an overfitting signature from the enlarged L2 candidate pool. Default remains `False`.
- **Family Regime/Side Diagnostics (env-gated DEBUG, zero side-effect on gate)**: `L1_FAMILY_REGIME_DIAG` env var activates `compute_family_regime_edge_diagnostics`, a per-(family, entry_regime_code) gross-edge measurement (bar-mean spread, Sharpe, block-bootstrap LCB, rank-IC) over `realized_event_results`. `split_side=True` additionally keys the same statistics by `(family, regime_code, side_label)` (`"long"`/`"short"` from the signed `side` column, zero-side rows excluded), populating `FamilyRegimeDiagnostics.by_family_regime_side` (`None` when unused or `side` column absent) to isolate directional edge asymmetry within a regime. All values are gross — comparison against `l1_breakeven_floor_bps` is required before treating a cell as tradeable.
- **Family Rejection Funnel (DEBUG-gated within the deployment registry table)**: `render_family_rejection_funnel` groups `[NOT PROMOTED]` evidence by family (parsed from `strategy_id`) and tabulates each family's `structural_reasons` counts (falling back to `quality_weight_zero` when reasons are empty), surfaced as `[L1-FAMILY-FUNNEL]` lines to distinguish data-driven rejection (e.g. `quality_weight_zero` from zero events) from edge-driven rejection (`negative_gross_edge`/`no_incremental_edge`) per candidate family.
- **Family Admission Gate (standalone evaluation utility, not wired into the promotion pipeline)**: `family_admission.evaluate_family_admission(promotions, family, ...)` admits a candidate family only when (a) `n_promoted_symbols >= MIN_ADMISSION_SYMBOLS` (default 3) distinct symbols show `lcb_bps` strictly greater than 0, and (b) `compute_trend_sleeve_corr` (mean Pearson r between the candidate's and the pooled trend-family bar-mean series, per symbol, over overlapping bars `>= MIN_CORR_OVERLAP_BARS`) does not exceed `MAX_TREND_SLEEVE_CORR` (0.50); a `None` correlation (insufficient overlap data) skips the correlation gate without blocking admission but is recorded in `reasons`. Consumed externally by measure-first diversification workflows — L1-level admission is necessary but not sufficient for production pool inclusion; see `docs/decisions/layer1-eh.md` for the standard subsequent verification (L2→L3 economic replay).
