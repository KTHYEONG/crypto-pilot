...
- `L0DiscoveryUnit` / `L0DiscoverySelection` (`discovery_units.py`): `unit_id`, `parent_recipe_id`, `kind`, `timeframe`, `family`, `variant`, `event_mask_2d`, `scope_symbols`, `cell_axes`, `cell_values`, `execution_style`, `fill_probability`, `adverse_selection_bps`, `horizon_bars`, `gate_evidence`로 구성된 standalone discovery contract. `build_l0_discovery_units()`는 `enable_discovery_unit_handoff=False`일 때 fail-closed로 빈 튜플을 반환하고, `select_l0_discovery_units()`는 Jaccard duplicate suppression + priority ranking, `project_discovery_units_to_panels()`는 `l0_*` metadata를 패널에 부착한다.
...
- `AlphaFoundryRuntimeConfig`: `enable_fast_discovery_timeframes` / `fast_discovery_timeframes` switch the discovery grid before native L0 gate execution; `observability_mode`, `debug_top_k_rows`, `debug_reject_bucket_rows`, `artifact_write_enabled`, `gate_schema` control runtime observability and schema validation; `enable_cost_aware_generation`, `exploration_budget_fraction`, `cost_prior_floor_by_tf`, `use_all_timeframes_in_l0` gate the search-space cost-prior screen in `run_alpha_foundry_l0_pipeline()`; `enable_failure_attribution` / `enable_conditional_l0_cells` / `enable_execution_arms` / `enable_horizon_sweep`(모두 기본 `False`) + `conditional_cell: ConditionalCellGateConfig` / `execution_arm: ExecutionArmConfig` / `horizon_sweep_bars`는 아래 Edge Failure Attribution & Conditional Alpha 절 참조 — **현재 `run_alpha_foundry_l0_pipeline()`에서 미소비(standalone 모듈, 배선 없음)**. `phase="l0"`는 `artifact_write_enabled=False` + `observability_mode="debug_log"`로 terminal JSON/CSV artifact emission만 허용한다.
- `AlphaFoundryRuntimeConfig`: 위 항목들에 더해 `enable_discovery_unit_handoff` / `max_discovery_units_for_l1` / `max_discovery_event_jaccard` / `min_discovery_unit_lcb_bps`가 discovery-unit handoff SSOT를 정의한다. 기본값은 비활성/보수적 차단.
- `run_alpha_foundry_l0_pipeline()`: `runtime_config`가 주어지면 (1) blueprint→`L0SearchCell` 생성 후 `apply_cost_prior_screen()`, (2) `evaluate_alpha_cheap_gate_batch()`(1차 저비용 스크리닝) → (3) canonical `evaluate_alpha_gate_batch()`(`capacity_score`/`regime_stability`/`tf_corroboration`/`entry_mode`/3단계 `handoff_tier` 산출, cheap-gate 통과분 재평가) → (4) diversity/budget는 canonical `handoff_tier`(cheap-gate 자체 `discovery_tier` 아님) 기준으로 `selected_for_l1` 결정 → (5) `update_search_policy_state()`로 search cell 상태 갱신 → (6) `observability_mode=="debug_log"`면 `src/domain/futures/observability/artifact_logging.py`의 terminal JSON/CSV emitters로 report/evidence payload를 남긴다. `tf_corroboration`은 `evidence_by_tf`가 주입돼야 `compute_tf_corroboration()`에서 0이 아닌 값을 가지며, `evaluate_alpha_gate_batch()`/`build_l0_signal_candidate()`(`pipeline.py`) 양쪽 다 `tf_fusion_index` 조회 키를 `(family, _strip_tf_suffix(variant, timeframe), timeframe)` 3-tuple로 정규화해야 `index_multi_timeframe_evidence()`의 색인과 일치한다.
...
- `fuse_multi_timeframe_evidence()` (`multi_tf_fusion.py`) — 동일 run epoch의 TF별 evidence DataFrame을 `(family, variant)`(TF 접미사 정규화 후) 기준으로 조인해, 다른 TF와의 부호일치도로 `corroboration_tier` 판정. `contradicted`는 `fused_conviction_score`를 강제 음수화(사실상 거부권), `corroborated`는 15% 컨빅션 부스트. `fuse_multi_timeframe_evidence_weighted()`는 `n_events`/`effective_n` 기반 coverage mass로 같은 출력 계약을 계산하는 weighted variant다.
- `shrink_l1_evidence_hierarchical()` applies family/timeframe shrinkage with `w = n_eff / (n_eff + prior_effective_n)`.

### Conditional Discovery Units [ADR_20260709_L0_CONDITIONAL_DISCOVERY_UNIT_HANDOFF]
- `discovery_units.py`: `L0DiscoveryUnit`/`L0DiscoverySelection`/`build_l0_discovery_units()`/`select_l0_discovery_units()`/`project_discovery_units_to_panels()`는 L0 조건부 셀, execution arm, horizon discovery를 위한 standalone fail-closed module이다. 현재 `run_alpha_foundry_l0_pipeline()`에 배선되지 않았고, `AlphaFoundryRuntimeConfig.enable_discovery_unit_handoff`는 기본 `False`다.
...
- `run_alpha_foundry_l0_gate(panels, bindings, recipes, aligned, cost_model, runtime_config, run_id, timeframe)` — panel→recipe matching, cheap gate → BH-lite/conviction floor → 버킷 다양성 선택 → 교차버킷 중복제거 실행, audit/gate 필터링. `artifact_write_enabled=True`면 `AlphaFoundryEvidenceRow` parquet + JSON 파일을 기록하고, `artifact_write_enabled=False` + `observability_mode="debug_log"`면 `src/domain/futures/observability/artifact_logging.py`를 통해 terminal JSON/CSV blocks만 남긴다.
...
- `_write_alpha_foundry_report(report, evidence_rows, report_dir, run_id)` — `artifact_write_enabled=True`인 경우에만 JSON(`{tf}_{timestamp}_report.json`, 집계)과 parquet(`{tf}_{timestamp}_evidence.parquet`, `AlphaFoundryEvidenceRow` 전건) 동시 기록. DEBUG terminal path는 `src/domain/futures/observability/artifact_logging.py`가 담당.
