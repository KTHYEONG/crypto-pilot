...
**Opportunity IC Mode**
- `l1_opp_ic_mode="time_series"` (default): Computes per-symbol Spearman IC over the event timeline (requires $\ge 3$ unique prediction values). Eliminates the cross-section synchrony requirement, enabling IC measurement for strategies with temporally sparse signals.
- `l1_opp_ic_mode="cross_section"`: Legacy per-bar cross-section IC (requires $\ge$ `l1_min_cross_section` symbols active per bar).
