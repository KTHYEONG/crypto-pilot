...
- **Regime Routing Diagnostics**: `RegimeRoutingDiagnostics` records `active_state_count=3`, `conditioning_path`, `mean_lift_bps`, `nw_tstat`, `fold_pass_ratio`, `bucket_hit_pct_by_fold`, `js_divergence_by_fold`. L2 logs `"[REGIME]"` as a compact 3-state table summary and `"[REGIME-L2]"` as the routing verdict.
