...
- **Reversal Kill-Switch** (env A/B, default off): `L2_REVERSAL_KILL` env activates pre-loop computation of `compute_reversal_risk_off_1d` (BTC trailing drawdown + negative momentum). At each rebalance bar $t$, if $risk\_off\_1d[t-1]$ is `True`, all sleeve `raw_mu` values are multiplied by `reversal_risk_off_floor=0.05` (selective hard de-gross, overrides soft cap and crisis_floor). Applied to all archetypes equally (market-wide reversal). Per-bar `(risk_off_flag, realized_price)` pairs collected for fold attribution decomposition via `risk_off_bars`, `risk_off_realized_price`, `risk_on_realized_price` in `Layer2FoldAttribution`.
