...
- **Phase B — fit-leg Deployment Calibration** (`risk_deployment.py`):
  ...
  - C4: `run_l2_awf(deploy_leverage=L^*)` → `apply_deployment(rets, L^*)` → `Layer2Result.deploy_leverage=L^*`. `exchange_leverage_cap` (default 10×) limits exchange feasibility. `l2_deploy_cvar_margin` knob.
