...
- **Positioning-Crowding Dampener** (env A/B, default off) `[ADR_20260703_L1_CROWD]`: `L2_POSITIONING_CROWDING_GATE` env precomputes per-symbol `compute_positioning_crowding_z_2d` (OI-build/LSR z-scores) and `compute_crowding_persistent_mask_2d` (persistence + recovery-cooldown state machine, mirrors `compute_reversal_risk_off_1d`). Sleeves whose family matches `_trend_arch_families` get `raw_mu` multiplied by `compute_crowding_dampener_mult(is_crowded, floor_mult=config.l2_crowding_floor_mult)`. `l2_crowding_floor_mult` has no default — must be set explicitly when the gate is enabled (raises `ValueError` otherwise). **Known defect**: the `_trend_arch_families` membership check compares display-style family labels (e.g. `"trend_donchian"`) against `_parse_meta_group_ids`'s variant-prefix output (e.g. `"donchian_72"`) — these never match, so this gate (and the pre-existing Trend-Efficiency Exposure Gate above, which shares the same check) never actually dampens any sleeve in production as currently wired. Root-cause fix (real sleeve→family mapping) is not yet implemented.
