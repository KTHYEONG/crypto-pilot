...
**L2 Purge/Embargo Activation:** `build_l2_simulation_folds` propagates the config's `purge_bars` and `embargo_bars` (auto-derived from `max_holding_bars * purge_safety_mult`) directly to `build_walk_forward_folds`. Purge inserts a gap between `fit_end` and `cal_start`; embargo inserts a gap between `cal_end` and `oos_start`. Previously hard-overridden to 0, this activation prevents fold-boundary label overlap in the anchored walk-forward scheme.
