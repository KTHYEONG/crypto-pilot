...
- **Reversal Kill-Switch** (env A/B, default off): `L2_REVERSAL_KILL` env activates pre-loop computation of `compute_reversal_risk_off_1d` (BTC trailing drawdown + negative momentum). Raw risk-off condition requires `persistence_bars` consecutive bars of drawdown+negative-momentum before the causal shift(1) produces the active mask. **Recovery hysteresis** (btc mode, shared state machine with panel mode): after persistent fires, the state stays True until `reversal_recovery_cooldown_bars` consecutive raw-off bars — exit counting uses raw signal (not persistent). At `recovery_cooldown_bars=0` (default), state tracks persistent byte-identically. At each rebalance bar $t$, if $risk\_off\_1d[t-1]$ is `True`, all sleeve `raw_mu` values are multiplied by `reversal_risk_off_floor` (selective hard de-gross, overrides soft cap and crisis_floor). Applied to all archetypes equally (market-wide reversal). Per-bar `(risk_off_flag, realized_price)` pairs collected for fold attribution decomposition via `risk_off_bars`, `risk_off_realized_price`, `risk_on_realized_price` in `Layer2FoldAttribution`. Config defaults are sourced from `RegimeConfig()`; env overrides `L2_REVERSAL_DD_WINDOW`, `L2_REVERSAL_DD_THRESHOLD`, `L2_REVERSAL_MOM_FAST`, `L2_REVERSAL_MOM_SLOW`, `L2_REVERSAL_RISK_OFF_FLOOR`, `L2_REVERSAL_PERSISTENCE_BARS`, `L2_REVERSAL_RECOVERY_COOLDOWN` via `_reversal_config_from_env()`. **Episode instrumentation**: `Layer2FoldAttribution.risk_off_episodes: tuple[ReversalEpisode, ...]` exposes contiguous risk-off runs (`start_idx`, `end_idx`, `realized_price`) extracted from the same per-bar mask, propagated through `Layer3Result`/`L3ReversalReplayResult` for downstream whipsaw diagnostics (`liquidity_stress_diag.compute_liquidity_stress_discriminative_power`).
- **Positioning-Crowding Dampener** (env A/B, default off) `[ADR_20260703_L1_CROWD]`: `L2_POSITIONING_CROWDING_GATE` env precomputes per-symbol `compute_positioning_crowding_z_2d` (OI-build/LSR z-scores) and `compute_crowding_persistent_mask_2d` (persistence + recovery-cooldown state machine, mirrors `compute_reversal_risk_off_1d`). Sleeves whose family matches `_trend_arch_families` get `raw_mu` multiplied by `compute_crowding_dampener_mult(is_crowded, floor_mult=config.l2_crowding_floor_mult)`. `l2_crowding_floor_mult` has no default — must be set explicitly when the gate is enabled (raises `ValueError` otherwise). **`_parse_meta_group_ids` fixed** `[ADR_20260704_L2_META_PARSER]`: the parser now splits on the canonical `"{family}:{variant}"` colon separator (`family, sep, variant = strategy_id.partition(":")`) before searching `variant` for the trailing `_{N}h` timeframe suffix, falling back to the legacy whole-string slice only when no colon is present. Previously the regex was applied to the raw `strategy_id` without splitting on `:`, so `family` came out polluted with the variant text (e.g. `"trend_donchian:donchian_72"` instead of `"trend_donchian"`), permanently failing `_trend_arch_families` membership for both this gate and the Trend-Efficiency Exposure Gate below, and fragmenting the regime×family×TF bucket keys (below) to per-variant granularity instead of per-family. Both env-gated mechanisms remain **default off** pending a dedicated economic-replay measurement now that they can actually fire; a same-config pipeline re-run confirmed the parser fix alone does not change L2/L3 headline metrics (bucket routing was not the source of the L3 holdout loss).
...
- **Phase B — fit-leg Deployment Calibration** (`risk_deployment.py`):
  - C1: fit-leg uses same OOS chain (rank→kelly→throttle→cost→funding), not equal-weight market avg.
   ...
   - C6: `diversification_gate_enabled`(bool, 기본 False): `compute_diversification_ratio`(Choueifaty-Coignard DR, `portfolio_constructor.py`)로 산출한 fit-leg per-bar DR 시계열의 최근구간 median이 전체 median 대비 하락하면 `l_final *= concentration_ratio`(선형 헤어컷, `concentration_floor`로 하한 클립) 적용, `binding="concentration_gate"`. `cov_mode`(sizing)와 무관한 orthogonal 제약 — realized MDD/CVaR 기반 L* 최적화 루프에 흡수되지 않도록 설계. `concentration_floor`는 활성화 시 명시 필수(미지정 시 `ValueError`). 기본 비활성 유지(§5 참조 — 오프라인 사전검증에서 discriminative power 미확인).
...
- **Whipsaw Decomposition (diag-gated, correction 2026-07-04)**: `realized_price_low_er` (Σ price delta where trailing ER < target), `trend_efficiency_corr` (corr(ER, price delta)), `mean_trend_efficiency` (fold-mean ER) are populated **only** when `(_trend_gate_enabled via L2_TREND_EFFICIENCY_GATE) or (_diag via l2_diag_attribution_enabled / L2_DIAG_ATTR)` is true — the `er_return_pairs` collection loop itself is skipped otherwise, silently leaving all three fields at their 0.0 default (looks like "zero trend efficiency" but actually means "uncollected"). `L2_DIAG_ATTR=1` must be set to get a real reading. `compute_mean_trend_efficiency(fold_attributions)` (`awf_sim.py`) aggregates these per-fold values into an oos-bar-weighted mean ER / mean corr across all fit/cal AWF folds, surfaced on `Layer2Result.mean_trend_efficiency` / `.trend_efficiency_corr` and rendered as a `[L2-REGIME]` line in `format_layer2_table`, plus a per-fold `ER:` column in the `[ FOLD DETAIL BREAKDOWN ]` table — enables direct fit/cal-vs-L3-OOS trend-character comparison (see `docs/architecture/layer3.md` §2.1).
- **Long/Short Realized-Price Split (always-on, no diag gate)**: `w_long = where(w>0, w, 0)` / `w_short = where(w<0, w, 0)` are computed once per rebalance (constant across the inner per-bar loop); each bar's existing `_bar_price = dot(w, bar_ret)` is additionally decomposed into `dot(w_long, bar_ret)` / `dot(w_short, bar_ret)` and accumulated into `Layer2FoldAttribution.realized_price_long` / `.realized_price_short` (plus `bars_long`/`bars_short` — count of bars with any nonzero long/short exposure). Unlike the ER fields above, this split needs no extra per-bar branch beyond the existing dot product, so it runs unconditionally regardless of `_diag`/`L2_DIAG_ATTR`. `compute_long_short_realized_price(fold_attributions)` sums these across fit/cal AWF folds (plain sum, same idiom as `compute_cost_drag_ratio`), surfaced on `Layer2Result.realized_price_long`/`.realized_price_short` and rendered as a `[L2-LONGSHORT]` line in `format_layer2_table` — enables direct fit/cal-vs-L3-OOS long/short P&L comparison (see `docs/architecture/layer3.md` §2.1).
- **Per-Symbol Long/Short Attribution (always-on, no diag gate)**: the same `w_long * bar_ret` / `w_short * bar_ret` elementwise products (pre-dot-product, i.e. before collapsing to the scalar `_bar_price_long`/`_bar_price_short` above) are additionally accumulated into per-symbol arrays `_attr_price_long_by_sym`/`_attr_price_short_by_sym` (shape `[n_sym]`). `_assemble_fold_attribution` zips these against `aligned.symbols` and filters contributors with `abs(value) <= 1e-12`, producing `Layer2FoldAttribution.realized_price_long_by_symbol`/`.realized_price_short_by_symbol` (`tuple[tuple[str, float], ...]`). `compute_long_short_price_by_symbol(fold_attributions)` merges (dict-sum, not weighted) these per-symbol tuples across fit/cal AWF folds — different folds may have different active symbol sets — surfaced on `Layer2Result` and rendered as `[L2-LONGSHORT-TOP]` lines (`_format_top_symbol_contributions`, top-5 by value, ascending for losers / descending for winners) in `format_layer2_table`. Purpose: distinguish a diffuse long-side P&L problem (every symbol contributes a little) from a concentrated one (a handful of symbols dominate), with concrete symbol names to act on (see `docs/architecture/layer3.md` §2.1 for the equivalent L3 rendering).
- **Per-Symbol Signal-vs-Sizing Decomposition (always-on, no diag gate)**: for a fixed watch-list `MAJOR_DIAG_SYMBOLS = ("BTCUSDT", "ETHUSDT", "BNBUSDT")`, each rebalance captures `MajorSymbolRebalanceSnapshot(t, symbol, raw_mu=mu_arr[i], weight=w[i], regime_code=_regime_now_for_cap, regime_risk_mult=_regime_risk_mult)` — the same `mu_arr`/`w`/regime-cap locals already computed for sizing, no new computation. Accumulated into `Layer2FoldAttribution.major_symbol_snapshots`. `summarize_major_symbol_signal_sizing(fold_attributions)` merges snapshots per symbol across fit/cal AWF folds and computes 5 ratios: `mu_bullish_pct` (fraction of bars with raw_mu>0), `weight_long_pct` (fraction with w>0), `stale_long_pct` (w>0 while raw_mu<=0 — architecturally bounded near-zero since `diagonal_kelly_weights` force-zeros deselected symbols regardless of no-trade-band), `regime_cap_engaged_pct` (fraction of long-held bars where `regime_risk_mult<1.0`), `mean_regime_risk_mult_when_long`. Surfaced on `Layer2Result.major_symbol_diag` and rendered as `[L2-MAJOR-DIAG]` lines (one per watched symbol) in `format_layer2_table`. Purpose: decompose a persistent-long-loss finding into 3 mutually exclusive causes — signal-direction lag (`mu_bullish_pct` alone stays high) vs sizing/hysteresis stuckness (`stale_long_pct`) vs regime-cap non-engagement (`regime_cap_engaged_pct` low) — see `docs/architecture/layer3.md` §2.1 for the L3 rendering and `docs/decisions/decisions.md` for the empirical finding this enabled.
...
| `l2_leverage_diversification_gate_enabled` | Enable DR-based concentration gate on L* (orthogonal to `cov_mode`) | False |
| `l2_leverage_concentration_recent_window_bars` | Recent-window bars for DR median comparison | 60 |
| `l2_leverage_concentration_floor` | Concentration-ratio floor clip; required (non-None) when gate enabled | None |
...
- **Portfolio sizing lever absorption**: Any sizing mechanism that uniformly scales weights per-rebalance (e.g. `l2_portfolio_cov_mode="correlated"`) is renormalized by `calibrate_deployment_leverage`'s MDD/CVaR-targeted L* search — the calibrator re-scales leverage to hit the same fit-leg risk budget regardless of the underlying sizing formula's conservatism. Only levers that change the *time distribution* of exposure (not just its magnitude) structurally escape this absorption. Confirmed under full-trial (200) L3 economic replay on clean ledger data: `correlated` mode's L* (2.0477) lands within 0.8% of `diagonal`'s (2.0646), and holdout CAGR/Sharpe/Sortino are uniformly worse under `correlated` — production default remains `"diagonal"`.
- **Concentration gate (DR) discriminative power unconfirmed**: The gate is implemented as an orthogonal L* constraint outside the MDD/CVaR search loop (escaping the absorption above by construction), but offline analysis of historical crash windows found realized diversification ratio does not reliably collapse during observed crises for this symbol universe — pairwise return correlation was measured lower during the crash than during calm periods, contrary to the "correlations converge to 1 in a crisis" assumption. `diversification_gate_enabled` therefore remains default-disabled pending new supporting evidence; do not enable in production without re-validating discriminative power on updated data.
