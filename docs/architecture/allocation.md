...
- **Signal OOS Rank IC gate (불변식):** signal 추천(`_recommendation_threshold_checks`)은 `oos_rank_ic >= min_oos_rank_ic`(기본 0.01)을 통과해야 합니다. IC 소스는 OOS-window(report_window) 기준 Spearman 상관계수입니다(`by_variant["oos_rank_ic"]`, 모든 OOS 메트릭과 동일 기간 기준). 이는 IS Spearman의 오버피팅 위험 배제(구현: `rule_diagnostics.py`).
- **IC t-stat gate (불변식, 2026-06-09 추가):** `_recommendation_threshold_checks`에 `ic_tstat >= min_ic_tstat`(기본 0.8) 게이트 추가. 공식: `t = IC × √(N−2) / √(1−IC²)` (Fisher 근사, df 보정). 점 추정 IC ≥ 0.01을 통과해도 표본 수(N)가 작으면 차단. **N 정합성 불변식:** IC는 OOS window Spearman(`by_variant` 소스)이므로 t-stat의 N도 OOS window 표본 수(`oos_ic_n`)를 사용해야 함. recommendation summary의 `oos_n`은 IS(fit+calibration) window count이므로 그대로 쓰면 t-stat이 부풀려짐 — substitution 시 `by_variant["oos_n"]`을 `oos_ic_n`으로 함께 carry. 구현: `_ic_tstat()` 헬퍼(`rule_diagnostics.py`).
- **q10_fail rate gate:** `max_variant_oos_q10_fail_rate`가 0.90(실질 무효) → **0.65**로 강화(2026-06-09). 이벤트의 65% 초과가 꼬리 손실 임계 미달인 변형은 `High Q10 Fail`로 차단.
