...
### 결정 (Decision)
벡터화된 가상 목표 가중치(Target Weight) 곱셈 방식 대신, 바이낸스 3분봉(`3m`) 프록시 체결과 현금·계약·수수료·펀딩비 결제를 통합한 [`SimulatedInventoryLedger`](file:///home/kth/crypto-pilot/src/mhs/execution/ledger.py)를 포트폴리오 PnL 산출의 기준 원장(Single Source of Truth)으로 채택함.
...
### 선정 이유 (Rationale)
3분봉은 기존 5분봉 대비 체결 정밀도를 약 +27% 향상시키며, 5개년 364개 심볼 패널을 약 7분 20초 만에 완주할 수 있어 계산 비용 대비 실거래 일치도가 극대화됨.

### 트레이드오프 (Trade-offs)
백테스트 연산 시간이 수 초에서 수 분으로 증가하므로, 효율적인 배치 메모리 관리와 NumPy 버퍼 스트리밍 기법이 요구됨.
...
### Selected Approach (채택된 접근 방식)
분기별 16개 확장 폴드 사이에 168시간의 퍼지/엠바고 구간을 물리적으로 제거하여 완전 격리. 또한 104개 탐색 경로의 표본 분산과 왜도/첨도를 반영하여 통계적 유의성을 검정하는 DSR 지표 산출.

### 선정 이유 (Rationale)
미래 참조 편향과 시계열 누출을 방지함. OOS 구간 15/16개 통과(93.8%) 및 DSR 0.73 달성을 통해 전략 성과의 통계적 유의성을 검증함.
...
### 선정 이유 (Rationale)
평활과 미평활 간의 근본적인 딜레마를 레짐 적응형으로 해결하여, 전략 기본 샤프를 0.53에서 1.08로 2배 이상 끌어올리고 비용 3배 스트레스 상황에서도 +0.83의 우수한 내구성을 입증함.
...
### 결정 (Decision)
외부 인바운드 포트를 완전 차단한 Tailscale VPN 사설망을 경유하고, Mozilla SOPS + Age 비대칭 암호화와 22개 전략 플래그 SHA256 불변 봉인(Seal)을 결합한 제로 트러스트 CI/CD 구축.
...
3. 라이브 데몬 구동 시 22개 전략 파라미터의 SHA256 해시를 대조하여 1바이트라도 불일치하면 [`ArtifactSealError`](file:///home/kth/crypto-pilot/src/live/errors.py)로 즉시 기동 중단.
