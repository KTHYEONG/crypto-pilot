...
| :--- | :--- | :--- | :--- | :--- |
| **ADR-01** | **체결 회계 모델** | • 벡터화 가상 곱셈<br>• L2/L3 틱 시뮬레이터 | **3분봉 모의 체결 원장 (`SimulatedInventoryLedger`)** | 8h 펀딩비 및 3-tier 수수료 실정산 반영, 체결 정밀도 +27% 향상. 계산 시간(~6분) 증가 수용 |
...
| **ADR-04** | **신호 결합 & 평활** | • 고정 롤링 평활<br>• 단일 모멘텀 의존 | **Causal Lag-1 적응형 평활 ($k=5$ 위원회 + 30% 캐리)** | 휩소 국면에서만 3행 평활 발동. 추세 진입 지연과 횡보장 슬리피지 간 트레이드오프 해소 |
| **ADR-05** | **저사양 클라우드 운용** | • 650개 심볼 전수 수집<br>• 수동 디스크 정리 | **Tail 증분 동기화(~2분) & 원자적 프루닝(430일 보존)** | 갱신 시간 29분 $\to$ ~2분으로 단축, OCI A1.Flex(2 OCPU/12GB) 컨테이너 `mem_limit: 3g` 예산 내 무인 운용 |
...
## ADR-05: 디스크 Tail 증분 갱신 및 원자적 보존 프루닝 (OCI A1.Flex Cloud Ops)

### Decision
저사양 단일 클라우드(OCI Ampere A1.Flex, 2 OCPU / 12GB 호스트 공유, `mhs-live` 컨테이너 `mem_limit: 3g`)에서 24/7 무인 가동을 위해 무거운 DB 인프라를 배제하고 로컬 Parquet 디스크 tail 증분 갱신과 430일(`SIGNAL_PANEL_WINDOW_DAYS`+30) 원자적 프루닝 체계를 구축함.

### Context
오라클 클라우드 환경에서 650개 선물 심볼의 전수 시세를 매 시간 동기화하면 갱신에만 29분이 소요되어 크론 타임아웃이 발생했습니다. 또한 축적되는 Parquet 파일이 15GB를 초과하여 디스크 고갈 위기에 직면했습니다. 2026-09-15에는 반대 방향 실패도 실측됨: 라이브-백테스트 패리티를 위해 신호 패널 창을 120일에서 400일로 넓힌 뒤(`9f6451fbd`) `mem_limit` 값을 그대로 둬 신호 계산 완주 시 피크 RSS 1.83GB가 1200m 한도에서 OOM(exit 137)을 일으킴 — 저사양 가정이 과거 값에 고정된 채 갱신되지 않아 실제로는 여유가 있는 12GB 호스트에서 불필요하게 매매가 중단된 사례.

### Alternatives
- **RDBMS / 시계열 DB (TimescaleDB / PostgreSQL)**: 유휴 상태에서도 1GB 이상의 메모리를 점유해 컨테이너 메모리 예산을 잠식.
- **매 사이클 전체 시세 재수집**: hourly 크론 허용 시간 초과.
- **컨테이너 메모리 한도 무제한**: 누수/폭주 시 호스트 전체(다른 프로젝트 컨테이너·systemd timer 포함) 동반 OOM 위험 -> 기각, 명시적 하드캡 유지.

### Selected Approach
1. 로컬 Parquet의 마지막 타임스탬프를 읽어 `max(tail - 2h, now - lookback)` 구간만 멀티스레드로 증분 패치.
2. 재생성 가능한 시계열(`ohlcv/1h`, `markPriceKlines/1h`, `funding`)에 한해 430일 초과 데이터를 원자적 임시 파일 대체(`tmp.replace(dest)`) 방식으로 안전하게 절단.
3. 컨테이너 `mem_limit`은 고정 저사양 가정이 아니라 실측 피크(데몬 상시 ~0.5GB + 신호 계산 피크 ~1.83GB + 여유)로 산정하고 호스트 RAM 대비 주기적으로 재검토함(`mhs-live: 3g`, `liquidation-collector: 768m`, 합계는 호스트 12GB의 40% 이내).

### Rationale
시세 갱신 소요 시간을 29분에서 약 2분으로 단축하고, 원자적 프루닝으로 디스크를 경량화하면서도 신호 패널 창 확장 같은 정당한 성장에는 메모리 예산을 실측 기반으로 함께 늘려, 저사양 가정이 자원 여유가 있는 호스트에서 매매를 불필요하게 막지 않도록 함.

### Trade-offs
복잡한 애드혹 SQL 쿼리는 불가능하며 분석 시 Parquet 파일을 메모리에 적재해야 함. 메모리 한도는 무제한이 아니므로 패널/유니버스가 계속 성장하면 주기적 재측정이 필요함.
