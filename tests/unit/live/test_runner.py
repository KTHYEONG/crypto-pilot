...
def _maybe_disable_orderbook_capture(monkeypatch, request):  # noqa: ARG001
    # Disable real orderbook capture (which sleeps) for all runner tests except the dedicated capture test
...
def live_env(monkeypatch, tmp_path):
    ...
    # disable orderbook capture for legacy tests to avoid real sleep
    import src.live.orderbook as ob_mod
