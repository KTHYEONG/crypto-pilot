...
def test_recorder_config_deadman_validators() -> None:
    """레코더 deadman 설정은 양수·타임아웃<간격을 강제한다."""
    import pytest
    ...
    with pytest.raises(ValueError, match="deadman_ping_interval_s"):
        MarketRecorderConfig(deadman_ping_interval_s=0)
    with pytest.raises(ValueError, match="deadman_ping_timeout_s"):
        MarketRecorderConfig(deadman_ping_interval_s=300.0, deadman_ping_timeout_s=300.0)


def test_recorder_heartbeat_is_failing_on_sampler_failures() -> None:
    """sampler 연속 실패가 있으면 failing, 아니면 정상이다."""
    import asyncio

    import pandas as pd

    from src.market_data.streams.recorder import _Heartbeat

    async def _run() -> None:
        hb = _Heartbeat(started_at=pd.Timestamp("2026-08-24 00:00Z"))
        assert hb.is_failing() is False
        hb._entries["book_ticker"]["consecutive_failures"] = 2
        assert hb.is_failing() is True
        hb._entries["book_ticker"]["consecutive_failures"] = "bad"
        hb._entries["premium_index"]["consecutive_failures"] = 0
        assert hb.is_failing() is False

    asyncio.run(_run())


def test_recorder_heartbeat_loop_pings_without_blocking(tmp_path, monkeypatch) -> None:
    """레코더 하트비트 루프는 핑을 보내고 sampler task와 함께 전진한다."""
    import asyncio
    import threading
    ...
    real_maybe_ping = deadman_mod.DeadmanPinger.maybe_ping

    def _spy(self, *, now, failing):
        ping_threads.append(threading.get_ident())
        return real_maybe_ping(self, now=now, failing=failing)

    monkeypatch.setattr(deadman_mod.DeadmanPinger, "maybe_ping", _spy)
    ...
    config = MarketRecorderConfig(deadman_ping_url="https://hc.example.com/rec", heartbeat_interval_s=0.01)
    asyncio.run(
        run_market_recorder(
            config, capture_root=tmp_path / "cap", liquidations_dir=tmp_path / "liq",
            shutdown=_Shutdown(), sleep=_sleep, fetch=_fetch,
            liquidation_runner=_noop,
        )
    )
    assert (tmp_path / "cap" / "recorder_heartbeat.json").exists()
    assert ping_threads, "heartbeat loop must attempt a deadman ping"
    assert all(t != loop_ident for t in ping_threads), "ping must run off the event loop"


def test_recorder_pinger_wired_from_config(tmp_path, monkeypatch) -> None:
    """run_market_recorder는 설정의 deadman URL로 pinger를 구성한다."""
    import asyncio
    ...
    real_pinger = deadman_mod.DeadmanPinger

    def _spy(**kwargs):
        seen.update(kwargs)
        return real_pinger(**kwargs)

    monkeypatch.setattr(deadman_mod, "DeadmanPinger", _spy)
    ...
    asyncio.run(
        recorder_mod.run_market_recorder(
            MarketRecorderConfig(deadman_ping_url="https://hc.example.com/rec"),
            capture_root=tmp_path / "cap2", liquidations_dir=tmp_path / "liq2",
            shutdown=_Shutdown(), sleep=_sleep, liquidation_runner=_noop,
        )
    )
    assert seen.get("interval_s") == 300.0


def test_recorder_main_reads_deadman_url_from_env(tmp_path, monkeypatch) -> None:
    """recorder_main은 LIVE_RECORDER_DEADMAN_PING_URL을 설정으로 전달한다."""
    from src.market_data.streams import recorder_main

    seen: dict = {}

    async def _fake_run(config, *, capture_root, liquidations_dir, shutdown):
        seen["url"] = config.deadman_ping_url.get_secret_value() if config.deadman_ping_url else None

    monkeypatch.setattr(recorder_main, "run_market_recorder", _fake_run)
    monkeypatch.setenv("LIVE_RECORDER_DEADMAN_PING_URL", "https://hc.example.com/rec")
    recorder_main.run_recorder(capture_root=tmp_path / "c", liquidations_dir=tmp_path / "l")
    assert seen["url"] == "https://hc.example.com/rec"
    monkeypatch.delenv("LIVE_RECORDER_DEADMAN_PING_URL")
    recorder_main.run_recorder(capture_root=tmp_path / "c", liquidations_dir=tmp_path / "l")
...
def test_recorder_heartbeat_ping_failure_is_logged_not_raised(tmp_path, monkeypatch) -> None:
    """레코더 핑 예외는 로그 후 루프가 계속된다."""
    import asyncio

    import src.live.deadman as deadman_mod
    from src.market_data.streams.recorder import MarketRecorderConfig, run_market_recorder

    def _boom(self, *, now, failing):
        raise RuntimeError("vendor down")

    monkeypatch.setattr(deadman_mod.DeadmanPinger, "maybe_ping", _boom)
    ...
    asyncio.run(
        run_market_recorder(
            config, capture_root=tmp_path / "cap3", liquidations_dir=tmp_path / "liq3",
            shutdown=_Shutdown(), sleep=_sleep, fetch=_fetch,
            liquidation_runner=_noop,
        )
    )
    assert (tmp_path / "cap3" / "recorder_heartbeat.json").exists()


def test_disabled_recorder_pinger_spawns_no_thread(tmp_path, monkeypatch) -> None:
    """URL 미설정 시 스레드 호출 자체를 건너뛰어 sampler 타이밍에 간섭하지 않는다."""
    import asyncio

    import src.live.deadman as deadman_mod
    from src.market_data.streams.recorder import MarketRecorderConfig, run_market_recorder

    assert deadman_mod.DeadmanPinger(url=None).enabled is False
    assert deadman_mod.DeadmanPinger(url="https://hc.example.com/x").enabled is True
    ...
    asyncio.run(
        run_market_recorder(
            MarketRecorderConfig(deadman_ping_url=None, heartbeat_interval_s=0.01),
            capture_root=tmp_path / "cap4", liquidations_dir=tmp_path / "liq4",
            shutdown=_Shutdown(), sleep=_sleep, fetch=_fetch,
            liquidation_runner=_noop,
        )
    )
...
def test_recorder_heartbeat_is_failing_on_flush_failures() -> None:
    """fetch가 정상이어도 flush 연속 실패(디스크 미기록)가 있으면 failing이다."""
    import asyncio
    ...
    async def _run() -> None:
        hb = _Heartbeat(started_at=pd.Timestamp("2026-08-24 00:00Z"))
        assert hb.is_failing() is False
        hb._entries["book_ticker"]["consecutive_flush_failures"] = 1
        assert hb.is_failing() is True
        hb._entries["book_ticker"]["consecutive_flush_failures"] = 0
        hb._entries["liquidations"] = {"consecutive_flush_failures": 2, "consecutive_failed_connections": 0}
        assert hb.is_failing() is True
