...
def test_build_bucket_reliability_side_preserved_in_output() -> None:
    reliability = build_bucket_reliability(
        regime=1, family="trend", tf="4h", side=-1,
        fit_edge_bps=18.0, cal_edge_bps=12.0,
        n_fit=30, n_cal=24, min_fit_n=15, min_cal_n=20,
        min_cal_lift_bps=8.0, min_reliability=0.55,
    )
    assert reliability.side == -1


def test_build_bucket_reliability_side_default_zero() -> None:
    reliability = build_bucket_reliability(
        regime=1, family="trend", tf="4h",
        fit_edge_bps=18.0, cal_edge_bps=12.0,
        n_fit=30, n_cal=24, min_fit_n=15, min_cal_n=20,
        min_cal_lift_bps=8.0, min_reliability=0.55,
    )
    assert reliability.side == 0
