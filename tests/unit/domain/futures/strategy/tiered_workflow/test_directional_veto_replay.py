        ...
        *,
        ...
        l2_veto_summary = (
            DirectionalVetoSummary(
                ...
                false_positive_rate=l2_fp, opportunity_cost=0.01,
                avoided_loss=max(l2_net_veto + 0.01, 0.02), net_veto_value=l2_net_veto,
            ),
        )
        return DirectionalVetoReplayResult(
            ...
            l3_cagr=0.12, l3_mdd=l3_mdd, l3_sharpe=l3_sharpe,
            l3_total_return=l3_total_return,
            l3_gate_passed=True, l3_blocker_reason="",
            l3_realized_price_long_by_symbol=l3_long_loss,
            l3_directional_veto_summary=l2_veto_summary,
            adoption_passed=False, blocker_reason="",
        )
...
class TestDirectionalVetoReplayRun:
    ...
    def test_returns_five_variants(
        ...
        mock_l2.return_value = MagicMock(
            spec=Layer2Result,
            cagr_hybrid=0.15, mdd_hybrid=0.08, turnover=0.5,
            average_gross_exposure=0.95, gate_passed=True, blocker_reason="",
            directional_veto_summary=(),
        )
        mock_l3.return_value = MagicMock(
            spec=Layer3Result,
            cagr=0.12, mdd=0.10, sharpe=1.2, total_return=0.05,
    ...
    def test_replay_with_baseline_parity(self) -> None:
        ...
        with patch(
            ...
            results = run_directional_veto_economic_replay(
                ...
                aligned=aligned, awf_folds=folds,
                holdout_span=(0, 100), config=config, caps=caps,
