...
def test_compute_coarse_precharge_bps_math() -> None:
    ...
    # total = 1.0 (fee) + 5.0 (spread) + 3.0 (impact) + 0.0 (tick) + 0.5 (latency) + 1.0 (funding) = 10.5 bps
...
def test_maker_share_impact_on_fee() -> None:
    ...
    # Difference in pre-charge should equal the taker fee - maker rebate difference (4.0 - (-2.0) = 6.0 bps)
