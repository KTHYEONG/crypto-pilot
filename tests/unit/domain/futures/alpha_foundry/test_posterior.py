...
class TestShrinkL1EvidenceHierarchical:
    def test_uses_family_prior_for_sparse_symbol(self) -> None:
        ...
        posterior = shrink_l1_evidence_hierarchical(
            raw_rows=rows, cost_model=cost, config=config
        )
    ...
    def test_raises_on_missing_required_columns(self) -> None:
        rows = pd.DataFrame({
            "recipe_id": ["r1"], "symbol": ["BTC"], "family": ["f"],
            "timeframe": ["4h"], "activation_context": ["pooled"],
            "fold_id": [0], "effective_weight": [1.0],
        })
