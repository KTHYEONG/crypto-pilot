...
def test_cash_carry_evaluation_preserves_ledger_and_provenance(
    ...
    monkeypatch.setattr("src.application.research.carry.evaluation.load_carry_market_data",
                        lambda symbol, start, end: data)
    monkeypatch.setattr(
        "src.application.research.carry.evaluation.cash_carry_data_hashes",
        lambda symbol: {
            "spot_ohlcv": "a" * 64,
            "perp_ohlcv": "b" * 64,
            "funding": "c" * 64,
    ...
    monkeypatch.setattr(
        "src.application.research.carry.evaluation.compute_code_hash",
        lambda: "e" * 64,
    )
