...
from src.domain.futures.opt_futures_utils.oos_evaluator import run_oos_margin_shared_portfolio
...
from src.domain.spot.opt_spot_utils.oos_evaluator import run_holdout_shared_cash_portfolio
...
def run_spot_portfolio(params: Dict[str, Any], reference_date: Optional[str] = None):
    ...
    FETCH_START, _START, IS_END, END = get_quarterly_window(reference_date)
    ...
    for sym in valid_symbols:
        ...
        data_maps[sym][f"oos_start_idx_{tf}"] = (
            int(m_oos.to_numpy().argmax()) if m_oos.any() else len(df)
        )
...
def main():
    ...
    parser.add_argument(
        "--reference-date", type=str, default=None, help="Reference date (YYYY-MM-DD)"
    )
