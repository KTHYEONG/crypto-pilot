...
def main() -> None:
    ...
    parser.add_argument(
        "--pytest-timeout", type=int, default=None,
    ...
    # correct; report keys match `_parse_file_coverage`'s path spellings.)
    cov_args = []
    if source_files:
    ...
    core_cmd = ["uv", "run", "pytest", *cov_args, "-m", "not slow", *test_files, *deselect_args, "-q", "--tb=line", *report_args]
    ...
    if pt_res.returncode == 0:
        ...
        last_err = [
            line for line in (pt_res.stdout or "").splitlines() if any(x in line for x in ("FAIL", "Error", "AssertionError"))
        ]
